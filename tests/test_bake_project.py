...
def bake_in_temp_dir(cookies, *args, **kwargs):
    """
    Return a result instance with a bunch of cookiecutter fields.
...
def test_bake_with_defaults(cookies):
    with bake_in_temp_dir(cookies) as result:
        ...
        assert 'pycookiecutter' in found_toplevel_files
...
def test_bake_withspecialchars_and_run_tests(cookies):
    ...
    with bake_in_temp_dir(cookies, extra_context={
            'full_name': 'name "quote" name'}) as result:
        assert result.project.isdir()
        run_inside_dir('python setup.py test', str(result.project)) == 0
...
def test_bake_without_travis_pypi_setup(cookies):
    with bake_in_temp_dir(cookies, extra_context={
            'use_pypi_deployment_with_travis': 'n'}) as result:
        result_travis_config = yaml.load(
            result.project.join(".travis.yml").open(), Loader=yaml.SafeLoader)
...
def test_bake_without_author_file(cookies):
    with bake_in_temp_dir(cookies, extra_context={
            'create_author_file': 'n'}) as result:
        found_toplevel_files = [f.basename for f in result.project.listdir()]
...
def test_bake_selecting_license(cookies):
    ...
    for license, target_string in license_strings.items():
        with bake_in_temp_dir(cookies, extra_context={
                'open_source_license': license}) as result:
...
def test_using_pytest(cookies):
    with bake_in_temp_dir(cookies, extra_context={
            'use_pytest': 'y'}) as result:
        assert result.project.isdir()
        test_file_path = result.project.join(
            'tests/test_pycookiecutter.py')
